...
#            print 'NVIDIA'
    ...
    env.Append( CC = 'g++')
    env.Append(CXXFLAGS = ['-Wall','-g',DEVFLAGS]) 
    env.Help(help)
    ...
    if OSX==1:
        ...
        env.Append(LIBS=['OpenCL'])
        if plat=='AMD':
            env.Append(CPPPATH=[OPENCL_DIR,AMD_SDK_PATH+'/include','/usr/include/CL'])
            env.Append(LIBPATH=[AMD_SDK_PATH+'/lib/x86_64'])
        elif plat=='Intel':
            env.Append(CPPPATH=[OPENCL_DIR,INTEL_SDK_PATH+'/include/CL'])
            env.Append(LIBPATH=[INTEL_SDK_PATH+'/lib64'])
        else:
            env.Append(CPPPATH=[OPENCL_DIR,OPENCL_DIR+'/CL',NVIDIA_SDK_PATH+'/OpenCL/common/inc'])
    env.Append(CPPPATH=OPENCL_DIR+'/OpenCLIntegration/')
    env.Append(LIBPATH=OPENCL_DIR+'/OpenCLIntegration/')
    env.Append(LIBS='OclWrapper')
    env.SConscript(OPENCL_DIR+'/OpenCLIntegration/SConstruct')
    return env
