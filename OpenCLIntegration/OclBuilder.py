...
#            print 'NVIDIA'
    ...
    DEVFLAGS=['-D'+dev,devidxflag]+env['KERNEL_OPTS']+['-DNRUNS='+nruns,'-DREF='+ref,vflag, memreadflag,devinfoflag,platinfoflag]+defflags
    ...
    env.Append(CXXFLAGS = ['-Wall',dbgflag,optflag,DEVFLAGS,KERNEL_OPTS]) 
...
#            env.Append(LIBS=['OclWrapper'])
        ...
        if plat=='AMD':
            env.Append(CPPPATH=[AMD_SDK_PATH+'/include','/usr/include/CL'])
            ...
            env.Append(CPPPATH=[OPENCL_DIR+'/CL',NVIDIA_SDK_PATH+'/OpenCL/common/inc'])
