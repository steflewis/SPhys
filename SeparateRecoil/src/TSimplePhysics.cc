...
,Rmean(0), Rstddev(1), accepted(0), rejected(0), nEvents(0), NewPrior(false), testPrior(true),fIndex(0), sampleIndex(0) 
{
...
TSimplePhysics::Prior(int fIndex)
{
 ...
 while (R_out_of_bounds == true){
   ...
   if ( (R[fIndex] >= (Rmean - (3*Rstddev))) && (R[fIndex] <= (Rmean + (3*Rstddev))) ){
     R_out_of_bounds = false;
     accepted++;
   }
   else
     rejected++;
...
{       
  ...
  bool     flag           = true;
  bool     pg_flag        = true;
  bool     Rflag          = true;  // False when R falls within bounds (mentioned in prior)
  int count = 0;
...
for( ; m > 0; m--)
{
  ...
  while (Rflag == true){
    ...
    if ( (trialR >= (Rmean - (3*Rstddev))) && (trialR <= (Rmean + (3*Rstddev))) ){
      Rflag = false;
    }
    else
      count++;
    ...
    if (count == 10){
      printf("Stuck at 10! fNest: %d\n",fNest);
      printf("R current: %lf\t trialR: %lf\t sigma:%lf\n",R[sampleIndex],trialR,sig);
      sig = sig/2;
    }else if (count == 100){
      printf("Stuck at 100! fNest: %d\n",fNest);
      printf("R current: %lf\t trialR: %lf\t sigma:%lf\n",R[sampleIndex],trialR,sig);
    }
    else if (count == 500){
      printf("Stuck at 500! fNest: %d\n",fNest);
      //sig *=2;
    }
  }
...
//____________________________________________________________________
void TSimplePhysics::SetMeanStdDev(float mean, float stdev){
