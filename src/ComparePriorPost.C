...
int main(){
  ...
  char * prior_filename = "/home/Stefanie/NewLikelihood/oldprior.txt";
  const char* filename = "/home/Stefanie/NewLikelihood/test_newLogLhood.root";
  ...
  TH2F *LogL_plot = new TH2F("LogL","LogL",100,0,1000000,100,-6000,-5950);
  ...
  // Read in data from posterior file
   ...
   for (Long64_t i = 0; i<nentries; i++){
       ...
       LogL_plot->Fill(allEvents[i],logL);
       // To here
   }
   // And here.
points.close();
  ...
  TCanvas *Logdist = new TCanvas("LogL_time","LogL_time");
  Logdist->SetFillStyle(0);
  Logdist->SetFillColor(0);
  Logdist->SetFrameFillStyle(0);
  Logdist->cd(1);
  LogL_plot->GetXaxis()->SetTitle("Event number");
  LogL_plot->GetYaxis()->SetTitle("LogL");
  LogL_plot->Draw("");
  Logdist->SaveAs("log_over_time.png");
